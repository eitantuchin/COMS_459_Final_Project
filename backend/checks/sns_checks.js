...
async function runSnsChecks(credentials) {
  ...
  const addCheck = (name, passed, message, assetIds = []) => {
    ...
    if (!passed) {
      assetIds.forEach(id => {
        ...
        const asset = allTopics.find(t => t.TopicArn === id) || 
                      topicDetails.find(t => t.attributes.TopicArn === id);
  ...
  let topicDetails = [];
  if (topicsExist) {
    topicDetails = await Promise.all(
      allTopics.map(topic => {
  ...
  regions.forEach(region => {
    regionStats[region].totalAssets = allTopics.filter(t => t.Region === region).length;
  });
