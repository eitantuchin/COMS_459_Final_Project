...
async function runElbChecks(credentials) {
  ...
  const addCheck = (name, passed, message, assetIds = []) => {
    ...
    if (!passed) {
      assetIds.forEach(id => {
        ...
        const asset = allClassicLbs.find(lb => lb.LoadBalancerName === id) ||
                      allModernLbs.find(lb => lb.LoadBalancerArn === id) ||
                      classicLbDetails.find(lb => lb.LoadBalancerName === id) ||
                      modernLbDetails.find(lb => lb.LoadBalancerArn === id);
  ...
  let classicLbDetails = [];
  let modernLbDetails = [];
  if (elbsExist) {
    classicLbDetails = await Promise.all(
    ...
    modernLbDetails = await Promise.all(
      allModernLbs.map(lb => {
  ...
  regions.forEach(region => {
    regionStats[region].totalAssets = allClassicLbs.filter(lb => lb.Region === region).length +
                                      allModernLbs.filter(lb => lb.Region === region).length;
  });
