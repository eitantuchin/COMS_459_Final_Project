...
async function runSqsChecks(credentials) {
  ...
  const addCheck = (name, passed, message, assetIds = []) => {
    ...
    if (!passed) {
      assetIds.forEach(id => {
        ...
        const asset = allQueueUrls.find(q => q.QueueUrl === id) || 
                      queueDetails.find(q => q.QueueUrl === id);
  ...
  let queueDetails = [];
  if (queuesExist) {
    queueDetails = await Promise.all(
  ...
  regions.forEach(region => {
    regionStats[region].totalAssets = allQueueUrls.filter(q => q.Region === region).length;
  });
