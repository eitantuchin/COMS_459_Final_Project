...
async function runLambdaChecks(credentials) {
  ...
  const addCheck = (name, passed, message, assetIds = []) => {
    ...
    if (!passed) {
      assetIds.forEach(id => {
        ...
        const asset = allFunctions.find(f => f.FunctionArn === id) || 
                      functionDetails.find(f => f.FunctionArn === id);
  ...
  let functionDetails = [];
  if (functionsExist) {
    functionDetails = await Promise.all(
  ...
  regions.forEach(region => {
    regionStats[region].totalAssets = allFunctions.filter(f => f.Region === region).length;
  });
